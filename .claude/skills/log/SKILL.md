---
name: log
description: Record a finished session from Tal's own words into logs/ and commit. Use when he says "log", "done", "I did ...", or dictates sets and does not want a new plan.
---

# /log

1. Parse what Tal said. Map names through `exercises/catalogue.yaml` (names + aliases).
   Unknown name → add it to the catalogue with `muscles` and the right `group`; ask ONE
   question only if you cannot tell what it works.
2. Shorthand you accept:
   ...
   - "did a random upper body thing, some rows and push ups" → log the named exercises with
     the sets he gives; missing counts stay missing.
...
followed_plan: false        # compared with plans/next.yaml for that date; omit if no plan existed
template: A                 # optional, if a template was followed
duration_min: 52
rpe: 7
exercises:
  - name: goblet squat
...
7. Commit: `git add -A && git commit -m "log: 2026-09-08" && git push`.
8. Reply with exactly one line: `Logged 2026-09-08 · legs · 5 exercises · 52 min · rpe 7.`
